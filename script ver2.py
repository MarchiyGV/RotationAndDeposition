...
class MyTakeStep:
   ...
   def __call__(self, x):
       ...
       while True:
           d = np.random.uniform(R_1, R_2)*np.random.choice([1,-1])
           if d+x[0]>=self.R_bounds[0] and d+x[0]<self.R_bounds[1]:
               x[0] += d
               print('MC switch: dR = %.1f' % d)
               while True:
                   d = np.random.uniform(k_1, k_2)*np.random.choice([1,-1])
                   if d+x[1]>=self.k_bounds[0] and d+x[1]<self.k_bounds[1]:
                       x[1] += d
                       print('MC switch: dk = %.2f' % d)
                       while True:
                           d = np.random.uniform(NR_1, NR_2)*np.random.choice([1,-1])
...
class MyBounds:
    def __init__(self, R_bounds, k_bounds, NR_bounds):
        self.xmax = np.array((R_bounds[1], k_bounds[1], NR_bounds[1]))
        self.xmin = np.array((R_bounds[0], k_bounds[0], NR_bounds[0]))
